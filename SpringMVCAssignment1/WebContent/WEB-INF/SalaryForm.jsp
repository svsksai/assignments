<%@ page language="java" contentType="text/html; charset=UTF-8"
    pageEncoding="UTF-8"%>
<%@taglib uri="http://www.springframework.org/tags" prefix="s" %>
<!DOCTYPE html>
<html>
<head>
<meta charset="UTF-8">
<title>SalaryForm</title>
<link rel="stylesheet"
	href="https://maxcdn.bootstrapcdn.com/bootstrap/4.3.1/css/bootstrap.min.css">
</head>
<body>
<h1 class="text-center bg-success p-2 m-3">Calculate Salary</h1>

<div class="text-right p-2 m-2"><a class="btn btn-danger" href="home.htm">Home</a></div>
<form action="calcsal.htm">
<table class=" col-md-4 table table-light table-striped  mx-auto">
<tr>
<td><s:message code="id"/></td>
<td><input type="number" name="eid" required />
</tr>
<tr>
<td><s:message code="name"/></td>
<td><input type="text" name="ename" required />
</tr>
<tr>
<td><s:message code="salary"/></td>
<td><input type="number" name="esal" required />
</tr>
<tr>
<td colspan="3" class="text-center p-2 m-3"><input type="submit" value="Calc Salary" /></td></tr>

</table>

</form>
</body>
</html>